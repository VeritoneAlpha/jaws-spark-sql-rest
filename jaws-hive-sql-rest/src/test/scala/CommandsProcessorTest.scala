...
class CommandsProcessorTest extends FunSuite {
  ...
  test("prepareCommands : ok-last command is a select") {
    ...
    val requiredCommandString = s"use databaseName;show tables;set hive.cli.print.header=true;select '$QUERY_DELIMITATOR';select\\s+([\\w]+)\\.\\* from \\( select \\* from table \\) ([\\w]+) limit 2"
   ...
   test("prepareCommands : ok-last command is not a select") {
    ...
    val requiredCommandString = s"use databaseName;show tables;set hive.cli.print.header=true;select '$QUERY_DELIMITATOR';show tables"
  ...
  test("prepareCommands : ok-one command") {
    ...
    val requiredCommandString = s"set hive.cli.print.header=true;select '$QUERY_DELIMITATOR';show databases"
