...
object Tables {
  ...
  def fromJavaMap(tables: java.util.Map[String, java.util.Map[String, Result]]): Tables = {
    val myScalaMap = tables.asScala.mapValues(_.asScala.toMap).toMap
    Tables(myScalaMap)
  }
  
  def fromMutableMap(tables: scala.collection.mutable.Map[String, scala.collection.mutable.Map[String, Result]]): Tables = {
