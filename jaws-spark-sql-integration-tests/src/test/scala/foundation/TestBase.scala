...
class TestBase extends FunSuite with BeforeAndAfterAll {
  ...
  def postRun(url: String, body: String) = {
    ...
    Await.ready(response, Inf).value.get match {
      case Success(r: HttpResponse) =>
        ...
        assert(queryID.nonEmpty, "Didn't receive a queryId")
  ...
  def waitforCompletion(queryId: String, retry: Int) = {
    ...
    while (status.equals("IN_PROGRESS") && mutableRetry != 0) {
      Thread.sleep(1000)
      val response: Future[HttpResponse] = pipeline(Get(s"${jawsUrl}queries?queryID=$queryId"))
      ...
      Await.ready(response, Inf).value.get match {
        case Success(r: HttpResponse) =>
          ...
          val result = gson.fromJson(data, classOf[Queries])
          status = result.queries(0).state
  ...
  def validataAllResultsFromNormalTable(queryId: String, strings : Boolean) {
    ...
    assert(flatResults.containsSlice(GenSeq("Ana", if (strings) "5" else 5, "f")), "Ana is missing")
    assert(flatResults.containsSlice(GenSeq("George",  if (strings) "10" else 10, "m")), "George is missing")
    assert(flatResults.containsSlice(GenSeq("Alina",  if (strings) "20" else 20, "f")), "Alina is missing")
    assert(flatResults.containsSlice(GenSeq("Paul",  if (strings) "12" else 12, "m")), "Paul is missing")
    assert(flatResults.containsSlice(GenSeq("Pavel",  if (strings) "16" else 16, "m")), "Pavel is missing")
    assert(flatResults.containsSlice(GenSeq("Ioana",  if (strings) "30" else 30, "f")), "Ioana is missing")
  }
